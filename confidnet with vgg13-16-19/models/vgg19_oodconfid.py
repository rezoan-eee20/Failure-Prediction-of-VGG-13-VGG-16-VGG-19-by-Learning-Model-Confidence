...
from confidnet.models.vgg19 import Conv2dSame


class VGG19OODConfid(AbstractModel):
    def __init__(self, config_args, device):
        ...
        self.conv7_dropout = nn.Dropout(0.4)
        self.conv8 = Conv2dSame(256, 256, 3)
        self.conv8_bn = nn.BatchNorm2d(256)
        self.maxpool3 = nn.MaxPool2d(2)

        self.conv9 = Conv2dSame(256, 512, 3)
        ...
        self.conv10_dropout = nn.Dropout(0.4)
        ...
        self.conv13_dropout = nn.Dropout(0.4)
        self.conv14 = Conv2dSame(512, 512, 3)
        self.conv14_bn = nn.BatchNorm2d(512)
        self.conv14_dropout = nn.Dropout(0.4)
        self.conv15 = Conv2dSame(512, 512, 3)
        self.conv15_bn = nn.BatchNorm2d(512)
        self.conv15_dropout = nn.Dropout(0.4)
        self.conv16 = Conv2dSame(512, 512, 3)
    ...
    def forward(self, x):
        ...
        out = self.conv13_dropout(out)
        out = F.relu(self.conv14(out))
        out = self.conv14_bn(out)
        out = self.conv14_dropout(out)
        out = F.relu(self.conv15(out))
        out = self.conv15_bn(out)
        out = self.conv15_dropout(out)
        out = F.relu(self.conv16(out))
        out = self.conv16_bn(out)
    ...
    def init_vgg19_params(self):
        ...
        vgg_layers = []
        for _layer in vgg19.features.children():
